...
def load_model(checkpoint_dir=None):
    ...
    if checkpoint_dir and Path(checkpoint_dir).is_dir():
        ...
        files = [f for f in os.listdir(checkpoint_path) if f.endswith('.pt')]
        if files:
            checkpoint_file = checkpoint_path / files[0]
            checkpoint = torch.load(checkpoint_file)
            # 尝试部分加载模型参数
            net.load_state_dict(checkpoint['model_state_dict'], strict=False)
            print(f"Loaded network weights from {checkpoint_file}")
            epoch = checkpoint.get('epoch', 0)
            train_loss_history = np.load(checkpoint_path / 'train_loss_history.npy').tolist() if 'train_loss_history.npy' in os.listdir(checkpoint_path) else []
            return net, epoch, train_loss_history
        else:
            print("No checkpoint files found, initializing network from scratch.")
    else:
        print("Checkpoint directory is invalid, initializing network from scratch.")
    return net, 0, []
